...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, ReactiveFormsModule, ZardCardComponent, ZardButtonComponent, ZardBadgeComponent, ...ZardTableImports, MxnCurrencyPipe],
...
export class RecipeDetailDialogComponent implements OnInit {
  ...
  getIngredientName(ingredientId: string): string {
    return this.supplyService.ingredients().find((ingredient) => ingredient._id === ingredientId)?.name ?? 'Sin selección';
  }

  getIngredientUnit(ingredientId: string): string {
    return this.supplyService.ingredients().find((ingredient) => ingredient._id === ingredientId)?.unit ?? '-';
  }

  getIngredientUnitCost(ingredientId: string): number {
    return this.supplyService.ingredients().find((ingredient) => ingredient._id === ingredientId)?.averageCost ?? 0;
  }

  getIngredientRowUnitCost(index: number): number {
    const row = this.form.controls.ingredients.at(index);
    const ingredient = this.supplyService.ingredients().find((item) => item._id === row.controls.ingredientId.value);
    const unitCostControl = Number(row.controls.unitCost?.value);
    return Number.isFinite(unitCostControl) && unitCostControl > 0 ? unitCostControl : (ingredient?.averageCost ?? 0);
  }
  ...
  getRowSubtotal(index: number): number {
    ...
    const ingredient = this.supplyService.ingredients().find((item) => item._id === row.controls.ingredientId.value);
    const unitCostControl = Number(row.controls.unitCost?.value);
    const unitCost = Number.isFinite(unitCostControl) && unitCostControl > 0 ? unitCostControl : (ingredient?.averageCost ?? 0);
  ...
  private createIngredientRow(recipeIngredientId = '', quantity = 0): IngredientRowForm {
    return new FormGroup({
      ingredientId: new FormControl(recipeIngredientId, { nonNullable: true, validators: [Validators.required] }),
      quantity: new FormControl(quantity, { nonNullable: true, validators: [Validators.required, Validators.min(0.01)] }),
  ...
  private applyRecipe(recipe: Recipe | null): void {
    ...
    recipe.ingredients.forEach((ingredient) => {
      this.form.controls.ingredients.push(this.createIngredientRow(ingredient.ingredient, ingredient.quantity));
