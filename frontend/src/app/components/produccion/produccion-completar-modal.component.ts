...
export class ProduccionCompletarModalComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    for (const item of batch.plannedIngredients) {
      this.actualIngredientsArray.push(this.fb.group({
    ...
    this.completionForm.valueChanges
  ...
  readonly actualTotalCost = computed(() => this.ingredientRows().reduce((acc, item) => acc + item.actualSubtotal, 0));
  readonly ingredientWasteCost = computed(() => this.ingredientRows().reduce((acc, item) => acc + item.wasteCost, 0));
  ...
  readonly yieldVariance = computed(() => {
    this.formTick();
    return Math.round((Number(this.completionForm.controls.actualProduced.value || 0) - Number(this.batch().plannedQuantity || 0)) * 1000) / 1000;
  });
  readonly yieldShortfallCost = computed(() => {
    const shortfall = Math.max(0, Number(this.batch().plannedQuantity || 0) - Number(this.completionForm.controls.actualProduced.value || 0));
  ...
  readonly projectedRecipeStock = computed(() => {
    ...
    return Math.round((currentStock + Number(this.completionForm.controls.actualProduced.value || 0)) * 1000) / 1000;
  });
  ...
  completeOrder(): void {
    ...
    void this.productionService.completeProductionBatch(this.batch()._id, {
