...
export class ProduccionDetalleModalComponent {
  ...
  readonly ingredientWasteCost = computed(() => {
    return Math.round((this.batch().actualIngredients?.reduce((acc, item) => acc + Number(item.wasteCost || 0), 0) || 0) * 100) / 100;
  });
  ...
  getActualQuantity(ingredientId: string): number | null {
    return this.batch().actualIngredients?.find((item) => item.ingredient === ingredientId)?.actualQuantity ?? null;
  }

  getWasteCost(ingredientId: string): number | null {
    return this.batch().actualIngredients?.find((item) => item.ingredient === ingredientId)?.wasteCost ?? null;
  }
}
